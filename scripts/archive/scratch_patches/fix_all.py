...
"""
    try:
        resp = requests.post(OLLAMA_URL, json={'model': MODEL, 'prompt': prompt, 'format': 'json', 'stream': False, 'options': {'temperature': 0.0}}, timeout=120)
        if resp.status_code == 200:
            data = resp.json()
            raw = data.get('response', '').strip()
            if not raw and 'thinking' in data: raw = data['thinking'].strip()
...
def process_company(company_name):
    ...
    for cat_code, qs in rubric.items():
        ...
        for i in range(0, len(binary_qs), 5): batches.append((cat_code, binary_qs[i:i+5]))
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=6) as executor:
        ...
        for future in concurrent.futures.as_completed(futures):
            res = future.result()
            if isinstance(res, list): all_answers.extend(res)
            
    ai_cats = {}
    human_qs = []
    for cat_code, qs in rubric.items(): ai_cats[cat_code] = {'questions': [], 'passed': 0, 'total': 0, 'full_name': cat_code}
    ...
    for ans in all_answers:
        ...
        entry = {'new_q_id': q_id, 'cat_code': cat, 'verdict': ans.get('verdict', 0), 'confidence': ans.get('confidence', 0.0), 'verbatim_citation': ans.get('citation')}
        if entry['confidence'] >= 0.85: ai_cats[cat]['questions'].append(entry)
        else:
    ...
    if row:
        ...
        cur.execute("UPDATE startup_extractions SET payload = %s::jsonb WHERE startup_id = %s", (json.dumps(payload), company_name))
        conn.commit()
    conn.close()
...
if __name__ == '__main__':
    conn = psycopg2.connect(DB_URL)
    cur = conn.cursor()
    cur.execute("SELECT startup_id FROM startup_extractions WHERE jsonb_array_length(payload->'human_questions') = 0 AND startup_id != 'Novagrid' AND startup_id NOT LIKE 'startup_test_%'")
