...
for type_name, pattern in type_patterns.items():
    ...
    # Look specifically at the 20% to 50% range (and overall >20%)
    lower_threshold = total_docs * 0.20
    upper_threshold = total_docs * 0.50
    
    delta_lines = []
    for norm_key, count in line_counts.most_common():
        # Check if between 20% and 50%
    ...
    if delta_lines:
        catalog_20_50[type_name] = {
            'total_files': total_docs,
            'range': f"20% - 50% ({int(lower_threshold)+1} to {int(upper_threshold)} files)",
            'lines': delta_lines
        }

out_file = "scaffolding_delta_20_to_50.txt"
with open(out_file, "w") as out:
    for type_name, data in catalog_20_50.items():
        out.write(f"\n{'='*90}\n")
        out.write(f"DOCUMENT DELIVERABLE: {type_name}\n")
        out.write(f"Files Analyzed: {data['total_files']} | Frequency Range: {data['range']}\n")
        out.write(f"New Scaffolding Blocks Discovered: {len(data['lines'])}\n")
        out.write(f"{'='*90}\n")
        
        for idx, item in enumerate(data['lines'], 1):
            out.write(f"[{item['count']}/{item['total']} files ({item['pct']:.1f}%)] {item['text']}\n")

print(f"Delta catalog written to {out_file}")
